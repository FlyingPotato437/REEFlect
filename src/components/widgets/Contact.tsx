...
import { IconMapPin, IconPhoneCall, IconUsers, IconClock, IconHeadset, IconHelp, IconMessages, type TablerIcon } from '@tabler/icons-react';
      ...
      <div className={`grid ${!content && !items ? 'md:grid-cols-1' : 'md:grid-cols-2'}`}>
        <div className="h-full pr-6">
          ...
          <ul className="mb-6 md:mb-0">
            {items &&
              items.map(({ title, description, icon: iconPropValue }, index) => {
                let IconComponent: TablerIcon | undefined = undefined;
                if (typeof iconPropValue === 'string') {
                  IconComponent = iconMap[iconPropValue];
                } else if (typeof iconPropValue === 'function') {
                  // If it's already a component function
                  IconComponent = iconPropValue;
                }
